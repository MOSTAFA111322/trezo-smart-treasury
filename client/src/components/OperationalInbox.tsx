...
export function OperationalInbox({ requests, onOpenRequests, onCreateRequest }: OperationalInboxProps) {
  ...
  return <section className="mt-5 rounded-2xl border bg-card p-5 sm:p-6" aria-labelledby="operational-inbox-title">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-amber-50 p-3 dark:bg-amber-950/25"><p className="text-xs text-muted-foreground">مسودات وإعادات</p><p className="mt-1 text-xl font-extrabold text-amber-800 dark:text-amber-300">{draftCount}</p></div><div className="rounded-xl bg-blue-50 p-3 dark:bg-blue-950/25"><p className="text-xs text-muted-foreground">قيد المراجعة</p><p className="mt-1 text-xl font-extrabold text-blue-800 dark:text-blue-300">{reviewCount}</p></div><div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950/25"><p className="text-xs text-muted-foreground">جاهز للتنفيذ</p><p className="mt-1 text-xl font-extrabold text-emerald-800 dark:text-emerald-300">{approvedCount}</p></div><div className="rounded-xl bg-red-50 p-3 dark:bg-red-950/25"><p className="text-xs text-muted-foreground">متأخر عن الموعد</p><p className="mt-1 text-xl font-extrabold text-red-800 dark:text-red-300">{overdueCount}</p></div></div>
    <div className="mt-4 grid gap-3">{actionable.length === 0 ? <div className="flex items-center gap-3 rounded-xl border border-dashed p-4 text-sm text-muted-foreground"><ShieldCheck size={18} className="text-primary"/><span>لا توجد سندات قيد المتابعة حالياً. يمكنك إنشاء طلب جديد عند الحاجة.</span></div> : actionable.map((request) => { const overdue = isOverdue(request); return <button type="button" key={request.id} onClick={onOpenRequests} className={`flex w-full items-center gap-3 rounded-xl border p-3 text-right transition-colors hover:border-primary/50 hover:bg-secondary/50 ${overdue ? "border-red-300 dark:border-red-900/70" : ""}`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${overdue ? "bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300" : "bg-secondary text-primary"}`}>{overdue ? <CalendarClock size={16}/> : <ClipboardCheck size={16}/>}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{request.title}</span><span className="mt-1 block text-xs text-muted-foreground">{request.referenceNumber} · {getStatusLabel(request.status, overdue)} · {getAction(request.status, overdue)}</span>{request.scheduledFor ? <span className="mt-1 block text-[11px] text-muted-foreground">موعد الصرف: {new Date(request.scheduledFor).toLocaleDateString("ar-SA")}</span> : null}</span><span className="shrink-0 text-left"><span className="block text-sm font-extrabold">{Number(request.amount).toLocaleString("en-US")} {request.currency}</span><span className="mt-1 block text-[11px] text-muted-foreground">{new Date(request.createdAt).toLocaleDateString("ar-SA")}</span></span><ArrowUpLeft size={15} className="shrink-0 text-muted-foreground"/></button>; })}</div>
    <p className="mt-4 flex items-center gap-1 text-[11px] text-muted-foreground"><CheckCircle2 size={13} className="text-primary"/>تتحدد إجراءات الاعتماد الفعلية من قائمة طلبات الصرف عبر فلتر «بانتظار إجراءي» ووفق الصلاحيات المعتمدة في النظام.</p>
  </section>;
}
