...
export function FinancialReportsPanel() {
  ...
  const exportReport = () => downloadCsv(financialReportCsv(rows), `trezo-financial-report-${scope}.csv`);
  const exportPdf = () => {
    ...
    doc.setFontSize(9); doc.text(`Scope: ${companyId ? `Company #${companyId}` : "All companies"} | ${fiscalYearId ? `Fiscal year #${fiscalYearId}` : "All fiscal years"}`, 14, 30);
    ...
    const drawTableHeader = () => { doc.setFontSize(7); headers.forEach((header, index) => doc.text(header, x[index], 40)); doc.line(14, 42, 284, 42); };
    drawTableHeader();
    rows.forEach((row, rowIndex) => {
      const pageRow = rowIndex % 28;
      if (rowIndex > 0 && pageRow === 0) { doc.addPage(); drawTableHeader(); }
      const y = 48 + pageRow * 7;
    ...
    doc.setFontSize(8); doc.text(`Generated ${new Date().toISOString()} | Records: ${rows.length}`, 14, 260);
  ...
  return <section className="mb-5 rounded-2xl border bg-card p-5 shadow-sm sm:p-6" aria-labelledby="financial-export-title">
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><div className="flex items-center gap-2 text-primary"><FileSpreadsheet size={20}/><h3 id="financial-export-title" className="font-display text-lg font-extrabold">تصدير التقرير المالي</h3></div><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">صدّر سجل طلبات الصرف بصيغة CSV أو PDF رسمي بعد التصفية حسب الشركة والسنة المالية. لا يشمل التصدير أي أسعار صرف تقديرية، ويُظهر PDF نطاق التقرير وتاريخ إنشائه.</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!rows.length || report.isLoading} onClick={exportReport} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"><Download size={17}/>{report.isLoading ? "جارٍ تجهيز التقرير…" : "تنزيل CSV"}</button><button type="button" disabled={!rows.length || report.isLoading} onClick={exportPdf} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-primary/30 px-4 py-3 text-sm font-bold text-primary transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"><FileText size={17}/>تنزيل PDF رسمي</button></div></div>
