...
export function InternalEmployeesPanel() {
  ...
  const linkUser = trpc.employees.linkUser.useMutation({ onSuccess: () => { setMessage("تم تحديث ربط حساب الدخول."); void employees.refetch(); }, onError: (error) => setMessage(`تعذر ربط الحساب: ${error.message}`) });
  const submit = () => {
  ...
  const activate = (employee: NonNullable<typeof employees.data>[number]) => updateEmployee.mutate({ id: employee.id, employeeNo: employee.employeeNo, fullName: employee.fullName, department: employee.department ?? undefined, jobTitle: employee.jobTitle ?? undefined, phone: employee.phone ?? undefined, operationalRole: employee.operationalRole as OperationalRole, isActive: true });
  ...
  return <section className="space-y-5 rounded-2xl border bg-card p-5">
    <div><h3 className="font-display text-lg font-extrabold">سجل الموظفين الداخلي</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">أضف أسماء الموظفين وأرقامهم الوظيفية بدون بريد إلكتروني. هذا السجل للتنظيم وتعيين الدور، ولا ينشئ حساب دخول تلقائياً.</p><p className="mt-2 rounded-xl bg-secondary px-3 py-2 text-xs font-semibold leading-5 text-secondary-foreground">ترتيب العمل: المحاسب ← المراجع المالي ← المدير المالي ← المدير العام والتنفيذ ← المدقق للمراجعة اللاحقة.</p></div>
    ...
    {users.data?.length === 1 ? <p className="rounded-xl bg-amber-500/10 px-3 py-2 text-xs font-semibold leading-5 text-amber-700 dark:text-amber-300">يوجد حساب دخول واحد فقط حالياً. لإضافة حساب آخر، يجب أن يسجل الموظف الدخول مرة واحدة من حساب مستقل؛ إضافة سجل موظف هنا لا تنشئ حساب دخول.</p> : null}
    {users.error ? <p className="text-sm text-destructive">تعذر تحميل حسابات الدخول: {users.error.message}</p> : null}
    {employees.isLoading ? <p className="text-sm text-muted-foreground">جارٍ تحميل سجل الموظفين…</p> : employees.error ? <p className="text-sm text-destructive">تعذر تحميل سجل الموظفين: {employees.error.message}</p> : employees.data?.length ? <div className="divide-y rounded-xl border">{employees.data.map((employee) => <div key={employee.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="font-bold">{employee.fullName} <span className="text-xs font-normal text-muted-foreground">({employee.employeeNo})</span></p><p className="text-xs text-muted-foreground">{[employee.department, employee.jobTitle, employee.phone].filter(Boolean).join(" · ") || "بيانات وظيفية غير مكتملة"}</p><p className="mt-1 text-xs font-semibold text-primary">{roleLabels[employee.operationalRole as OperationalRole]} · {employee.isActive ? "نشط" : "غير نشط"}</p></div><div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-64"><select aria-label={`حساب الدخول للموظف ${employee.fullName}`} value={String(employee.linkedUserId ?? 0)} disabled={linkUser.isPending || !users.data?.length} onChange={(event) => { const selected = Number(event.target.value); linkUser.mutate({ employeeId: employee.id, userId: selected > 0 ? selected : null }); }} className="rounded-lg border bg-background px-3 py-1.5 text-xs font-semibold"><option value="0">بدون ربط بحساب دخول</option>{users.data?.map((user) => <option key={user.id} value={user.id}>{user.name || user.email || `مستخدم #${user.id}`}</option>)}</select><div className="flex flex-wrap gap-2"><button type="button" onClick={() => edit(employee)} className="rounded-lg border px-3 py-1.5 text-xs font-bold">تعديل</button>{employee.isActive ? <button type="button" onClick={() => deactivate(employee)} disabled={updateEmployee.isPending} className="rounded-lg border border-destructive/40 px-3 py-1.5 text-xs font-bold text-destructive">تعطيل</button> : <button type="button" onClick={() => activate(employee)} disabled={updateEmployee.isPending} className="rounded-lg border border-primary/40 px-3 py-1.5 text-xs font-bold text-primary">تفعيل الموظف</button>}</div></div></div>)}</div> : <div className="rounded-xl bg-secondary p-4 text-sm text-muted-foreground">لا يوجد موظفون مضافون بعد. ابدأ بإضافة أول موظف بالرقم الوظيفي والاسم.</div>}
  </section>;
}
