...
export function RequestDetailPanel({ rows, currencies, selectedRequestId: controlledSelectedId, onSelectedRequestIdChange }: Props) {
  ...
  const printSelected = (kind: "order" | "receipt" | "report" | "archive") => {
    ...
    const date = selected.scheduledFor ? new Date(selected.scheduledFor).toLocaleString("ar-SA") : "غير محدد";
    ...
    const fields = { reference: escapeHtml(selected.referenceNumber), title: escapeHtml(selected.title), description: escapeHtml(selected.description ?? "—"), company: escapeHtml(companyName), legalName: escapeHtml(companyLegalName), tax: escapeHtml(companyTaxNumber), phone: escapeHtml(companyPhone), address: escapeHtml(companyAddress), beneficiary: escapeHtml(beneficiaryName), beneficiaryPhone: escapeHtml(beneficiaryPhone), channel: escapeHtml(channelName), bank: escapeHtml(bankAccountLabel), date, amount, status };
    const isArchive = kind === "archive";
    const heading = kind === "order" ? "أمر صرف رسمي" : kind === "receipt" ? "سند صرف حديث" : kind === "report" ? "سند صرف مختصر" : "بطاقة أرشفة داخلية";
    ...
    const archiveRows = ["المحاسب", "المراجع", "المدير المالي", "المدير العام", "المدقق"].map((label, index) => `<tr><td>${label}</td><td>${index < (selected.status === "executed" ? 5 : selected.status === "approved" ? 3 : selected.status === "review" ? 1 : 0) ? "مكتمل" : "بانتظار الإجراء"}</td></tr>`).join("");
    const approvalEvents = (workflow.data ?? []).filter((event) => event.toStatus !== "draft");
    const signatures = approvalEvents.length ? approvalEvents.map((event) => `<div class="signature"><strong>اعتماد إلكتروني مسجل</strong><br/>${escapeHtml(event.actorName ?? `المستخدم رقم ${event.actorId}`)}<br/><small>${escapeHtml(event.toStatus)} · ${escapeHtml(new Date(event.createdAt).toLocaleString("ar-SA"))}</small></div>`).join("") : `<div class="signature">لا توجد اعتمادات مسجلة بعد</div>`;
    ...
    const content = isArchive ? archive : kind === "order" ? official : kind === "receipt" ? modern : dataTable;
