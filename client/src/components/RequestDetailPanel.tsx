...
export function RequestDetailPanel({ rows, currencies, selectedRequestId: controlledSelectedId, onSelectedRequestIdChange }: Props) {
  ...
  const companies = trpc.entities.companies.list.useQuery(undefined, { enabled: Boolean(selected) });
  ...
  const workflow = trpc.requests.workflow.useQuery({ requestId: selected?.id ?? 0 }, { enabled: Boolean(selected) });
  ...
  const printSelected = (kind: "order" | "receipt" | "report" | "archive") => {
    ...
    const title = isArchive ? `بطاقة أرشفة داخلية ${selected.referenceNumber}` : isReceipt ? `سند صرف حديث ${selected.referenceNumber}` : isReport ? `سند صرف مختصر ${selected.referenceNumber}` : `سند صرف رسمي ${selected.referenceNumber}`;
    const heading = isArchive ? "بطاقة أرشفة داخلية" : isReceipt ? "سند صرف حديث" : isReport ? "سند صرف مختصر" : "سند صرف رسمي للشركة";
    ...
    const beneficiary = beneficiaries.data?.find((item) => item.id === selected.beneficiaryId);
    const beneficiaryName = beneficiary?.name ?? "—";
    const beneficiaryPhone = beneficiary?.phone ?? "غير مسجل";
    const company = companies.data?.find((item) => item.id === selected.companyId);
    ...
    const approvalEvents = (workflow.data ?? []).filter((event) => event.toStatus !== "draft");
    const electronicStamp = selected.status === "executed" ? `<div class="system-stamp">ختم TREZO الإلكتروني<br/><small>مكتمل الاعتماد · ${escapeHtml(selected.referenceNumber)}</small></div>` : "";
    const signatureRows = approvalEvents.length > 0
      ? approvalEvents.map((event) => `<div class="signature"><strong>اعتماد إلكتروني مسجل</strong><br/>${escapeHtml(event.actorName ?? `المستخدم رقم ${event.actorId}`)}<br/><small>${escapeHtml(event.toStatus)} · ${escapeHtml(new Date(event.createdAt).toLocaleString("ar-SA"))}</small></div>`).join("")
      : `<div class="signature"><strong>لا توجد اعتمادات مسجلة بعد</strong><br/><small>يظهر التوقيع بعد تنفيذ الإجراء من المستخدم المخوّل</small></div>`;
    popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4;margin:16mm}body{font-family:Arial,sans-serif;color:#17202a;padding:28px;line-height:1.8}.brand{text-align:center;border-bottom:3px solid #176b54;padding-bottom:12px}.logo{max-width:120px;max-height:70px;object-fit:contain;margin:0 auto 8px;display:block}.brand h1{margin:0;color:#176b54;font-size:24px}.brand h2{font-size:16px;font-weight:normal;margin:4px 0 0}.company-meta{font-size:11px;color:#59656e;margin:6px 0 0}.trezo-mark{font-size:11px;color:#176b54;font-weight:bold;margin:2px 0 0}.stamp{text-align:right;margin-top:14px;color:#59656e;font-size:12px}table{width:100%;border-collapse:collapse;margin-top:22px}td{border:1px solid #cfd6dd;padding:11px}td:first-child{width:28%;font-weight:bold;background:#f5f7f9}.workflow-title{margin:26px 0 4px;color:#176b54;font-size:16px}.workflow td:first-child{width:50%}.notice{border-right:4px solid #d5a746;background:#fff9e9;padding:10px 14px;margin-top:22px;font-size:13px}.system-stamp{display:inline-block;margin-top:24px;border:2px solid #176b54;color:#176b54;padding:8px 18px;text-align:center;font-weight:bold;transform:rotate(-2deg)}.system-stamp small{font-weight:normal}.signatures{display:grid;grid-template-columns:repeat(2,1fr);gap:28px;margin-top:38px;text-align:center}.signature{min-height:68px;border-top:1px solid #5b6570;padding-top:10px;font-size:13px}.signature small{color:#59656e}footer{margin-top:34px;padding-top:10px;border-top:1px solid #cfd6dd;color:#59656e;font-size:10px;text-align:center}@media print{body{padding:12mm}}</style></head><body><header class="brand">${companyLogoUrl ? `<img class="logo" src="${escapeHtml(companyLogoUrl)}" alt="شعار ${escapeHtml(companyName)}"/>` : ""}<h1>${escapeHtml(heading)}</h1><h2>${escapeHtml(companyLegalName)}</h2><p class="company-meta">${escapeHtml(companyAddress)} · ${escapeHtml(companyPhone)} · الرقم الضريبي: ${escapeHtml(companyTaxNumber)}</p><p class="trezo-mark">الخزينة الذكية TREZO</p></header><div class="stamp">${isArchive ? "نسخة أرشيفية داخلية" : "نسخة رسمية للشركة"}<br/>تاريخ الإصدار: ${new Date().toLocaleDateString("ar-SA")}</div><table><tr><td>الرقم المرجعي</td><td>${escapeHtml(selected.referenceNumber)}</td></tr><tr><td>الجهة</td><td>${escapeHtml(companyName)}</td></tr><tr><td>الاسم القانوني</td><td>${escapeHtml(companyLegalName)}</td></tr><tr><td>الهاتف الرسمي</td><td>${escapeHtml(companyPhone)}</td></tr><tr><td>العنوان الرسمي</td><td>${escapeHtml(companyAddress)}</td></tr><tr><td>الرقم الضريبي</td><td>${escapeHtml(companyTaxNumber)}</td></tr><tr><td>البيان الطلب</td><td>${escapeHtml(selected.title)}</td></tr><tr><td>الوصف</td><td>${escapeHtml(selected.description ?? "—")}</td></tr><tr><td>المبلغ</td><td>${escapeHtml(String(selected.amount))} ${escapeHtml(selected.currency)}</td></tr><tr><td>المستفيد</td><td>${escapeHtml(beneficiaryName)}</td></tr><tr><td>هاتف المستفيد</td><td>${escapeHtml(beneficiaryPhone)}</td></tr><tr><td>قناة الصرف</td><td>${escapeHtml(channelName)}</td></tr><tr><td>الحساب البنكي</td><td>${escapeHtml(bankAccountLabel)}</td></tr><tr><td>موعد الصرف</td><td>${escapeHtml(selected.scheduledFor ? new Date(selected.scheduledFor).toLocaleString("ar-SA") : "غير محدد")}</td></tr><tr><td>الحالة</td><td>${escapeHtml(statusPresentation?.label ?? selected.status)}</td></tr><tr><td>الإجراء التالي</td><td>${escapeHtml(nextActor ? `${nextActor.role} — ${nextActor.action}` : "لا يوجد إجراء تالٍ")}</td></tr></table><h3 class="workflow-title">مسار الاعتماد الخماسي</h3><table class="workflow"><tr><td>المرحلة</td><td>الحالة</td></tr>${stageRows}</table><p class="notice">${escapeHtml(notice)}</p>${electronicStamp}<div class="signatures">${signatureRows}</div><footer>المرجع: ${escapeHtml(selected.referenceNumber)} · الحالة عند الطباعة: ${escapeHtml(selected.status)} · مستند مولد من TREZO</footer><script>window.onload=()=>window.print()</script></body></html>`);
  ...
  return <div className="mt-5 rounded-2xl border bg-card p-5">
    ...
    {selected ? <div className="mt-5 rounded-xl bg-secondary/50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold">{selected.referenceNumber} · الحالة: {statusPresentation?.label ?? selected.status}</p><p className="mt-1 text-xs text-muted-foreground">{nextActor ? `التالي: ${nextActor.role} — ${nextActor.action}` : "اكتملت الإجراءات المسجلة لهذا الطلب."}</p></div><button onClick={() => void copyReference()} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold" title="نسخ الرقم المرجعي"><Clipboard size={14}/> {copyLabel}</button><button onClick={() => printSelected("order")} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold"><Building2 size={14}/> رسمي</button><button onClick={() => printSelected("receipt")} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold"><ReceiptText size={14}/> حديث</button><button onClick={() => printSelected("report")} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold"><FileText size={14}/> مختصر</button><button onClick={() => printSelected("archive")} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold"><Archive size={14}/> أرشيف داخلي</button>{canEdit && !editing ? <button onClick={() => setEditing(true)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-bold text-primary-foreground"><Pencil size={14}/> تعديل الطلب</button> : null}</div>
