...
export function RequestDetailPanel({ rows, currencies }: Props) {
  ...
  const printSelected = () => {
    ...
    popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>أمر صرف ${escapeHtml(selected.referenceNumber)}</title><style>body{font-family:Arial,sans-serif;color:#17202a;padding:36px;line-height:1.8}h1{text-align:center;margin:0 0 8px}h2{text-align:center;font-size:16px;font-weight:normal;margin:0 0 28px}table{width:100%;border-collapse:collapse;margin-top:22px}td{border:1px solid #cfd6dd;padding:11px}td:first-child{width:28%;font-weight:bold;background:#f5f7f9}.signatures{display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-top:70px;text-align:center}.signature{border-top:1px solid #5b6570;padding-top:10px;font-size:13px}@media print{body{padding:12mm}}</style></head><body><h1>أمر صرف</h1><h2>الخزينة الذكية TREZO</h2><table><tr><td>الرقم المرجعي</td><td>${escapeHtml(selected.referenceNumber)}</td></tr><tr><td>بيان الطلب</td><td>${escapeHtml(selected.title)}</td></tr><tr><td>الوصف</td><td>${escapeHtml(selected.description ?? "—")}</td></tr><tr><td>المبلغ</td><td>${escapeHtml(String(selected.amount))} ${escapeHtml(selected.currency)}</td></tr><tr><td>الحالة</td><td>${escapeHtml(selected.status)}</td></tr></table><div class="signatures"><div class="signature">المحاسب</div><div class="signature">المراجع</div><div class="signature">المدير المالي</div><div class="signature">المدير العام</div></div><script>window.onload=()=>window.print()</script></body></html>`);
