...
describe("reference sequence atomic reservation", () => {
  ...
  it("allows only one of two concurrent requests to reserve the same sequence value", async () => {
    let sequenceReads = 0;
    let releaseSequenceReads: (() => void) | undefined;
    const bothReadSequence = new Promise<void>((resolve) => { releaseSequenceReads = resolve; });
    let reserved = false;
    const makeTransaction = (): Transaction => {
      let selectCalls = 0;
      const transaction: Transaction = {
        select: vi.fn(() => ({ from: vi.fn(() => ({ where: vi.fn(() => ({ limit: vi.fn(async () => { selectCalls += 1; if (selectCalls === 2) { sequenceReads += 1; if (sequenceReads === 2) releaseSequenceReads?.(); await bothReadSequence; return [{ id: 7, fiscalYearId: 7, prefix: "TR", nextValue: 12, padding: 4 }]; } return [{ id: 7, year: 2026 }]; }) })) })) })),
    ...
    const callerA = appRouter.createCaller(context(1));
    const callerB = appRouter.createCaller(context(2));
    const input = { companyId: 1, beneficiaryId: 1, channelId: 1, fiscalYearId: 7, title: "طلب متنافس", amount: 100, currency: "SAR" };

    const results = await Promise.allSettled([callerA.requests.createDraft(input), callerB.requests.createDraft(input)]);
