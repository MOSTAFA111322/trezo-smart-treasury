...
export const appRouter = router({
  ...
  entities: router({
    ...
    beneficiaries: router({
      ...
      update: protectedProcedure.input(z.object({ id: z.number().int().positive(), companyId: z.number().int().positive(), name: z.string().min(2).max(180), type: z.enum(["individual", "organization"]), taxNumber: z.string().max(80).optional(), phone: z.string().max(40).optional(), email: z.string().email().optional() })).mutation(async ({ input }) => { const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(beneficiaries).set({ companyId: input.companyId, name: input.name, type: input.type, taxNumber: input.taxNumber, phone: input.phone, email: input.email }).where(eq(beneficiaries.id, input.id)); return { success: true }; }),
