...
export const appRouter = router({
  ...
  entities: router({
    ...
    beneficiaries: router({
      ...
      update: protectedProcedure.input(z.object({ id: z.number().int().positive(), companyId: z.number().int().positive(), name: z.string().min(2).max(180), type: z.enum(["individual", "organization"]), taxNumber: z.string().max(80).optional(), phone: z.string().max(40).optional(), email: z.string().email().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(beneficiaries).set({ companyId: input.companyId, name: input.name, type: input.type, taxNumber: input.taxNumber, phone: input.phone, email: input.email }).where(eq(beneficiaries.id, input.id)); await writeEntityAudit(db, ctx.user.id, "beneficiary.update", "beneficiary", input.id, input); return { success: true }; }),
      remove: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { if (ctx.user.role !== "admin") throw new Error("صلاحية المدير مطلوبة"); const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(beneficiaries).set({ isActive: false }).where(eq(beneficiaries.id, input.id)); await writeEntityAudit(db, ctx.user.id, "beneficiary.deactivate", "beneficiary", input.id, { isActive: false }); return { success: true }; }),
    }),
    banks: router({
      ...
      update: protectedProcedure.input(z.object({ id: z.number().int().positive(), name: z.string().min(2).max(160), swiftCode: z.string().max(40).optional(), country: z.string().max(80).optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(banks).set({ name: input.name, swiftCode: input.swiftCode, country: input.country }).where(eq(banks.id, input.id)); await writeEntityAudit(db, ctx.user.id, "bank.update", "bank", input.id, input); return { success: true }; }),
      remove: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { if (ctx.user.role !== "admin") throw new Error("صلاحية المدير مطلوبة"); const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(banks).set({ isActive: false }).where(eq(banks.id, input.id)); await writeEntityAudit(db, ctx.user.id, "bank.deactivate", "bank", input.id, { isActive: false }); return { success: true }; }),
    }),
    channels: router({
      ...
      update: protectedProcedure.input(z.object({ id: z.number().int().positive(), name: z.string().min(2).max(120), code: z.string().min(2).max(32), description: z.string().max(500).optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(disbursementChannels).set({ name: input.name, code: input.code, description: input.description }).where(eq(disbursementChannels.id, input.id)); await writeEntityAudit(db, ctx.user.id, "channel.update", "disbursement_channel", input.id, input); return { success: true }; }),
      remove: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { if (ctx.user.role !== "admin") throw new Error("صلاحية المدير مطلوبة"); const db = await getDb(); if (!db) throw new Error("قاعدة البيانات غير متاحة"); await db.update(disbursementChannels).set({ isActive: false }).where(eq(disbursementChannels.id, input.id)); await writeEntityAudit(db, ctx.user.id, "channel.deactivate", "disbursement_channel", input.id, { isActive: false }); return { success: true }; }),
    }),
  }),
  calendar: router({
